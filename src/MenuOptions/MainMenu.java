...
import StartAndMore.UsefulMethods;
import DummyData.SyntheticData;
import Models.Student;


public class MainMenu {
    UsefulMethods input=new UsefulMethods();
    SyntheticData synthetics=new SyntheticData();
    boolean b=false;
    
    //CONSTRUCTOR
    public MainMenu(){    
    }
    //ASKING THE USER IF HE WANTS TO RUN SYNTHETICS DATA
    public void syntheticMenu(){
        System.out.println("DO YOU WANT TO RUN SYNTHETIC DATA? Y/N");
        if(input.yesOrNo()){
            SyntheticData.create();
            menu();
        }else{
            menu();
        }
    ...
    public void printAllLists(){
        ...
        switch(x){
            ...
            case 5:
                input.underline();
                printList.printStudentsCourses();
                seeMoreLists();
                break;
            case 6:
                input.underline();
                printList.printTrainersPerCourses();
                seeMoreLists();
                break;
            case 7:
                input.underline();
                printList.printAssignmentsPerCourse();
                seeMoreLists();
                break;
            case 8:
                input.underline();
                printList.printAssignmentsPerStudents();
                seeMoreLists();
                break;
            case 9:
                input.underline();
                printList.printStudentsWithMoreThanOneCourse();
                seeMoreLists();
                break;
            case 10:
                menu();        
